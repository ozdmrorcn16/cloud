...
- [ ] **Step 1: Migrasyon dosyasini yaz** (ozet; tam govde dosyada):
...
- [ ] **Step 2: MCP `apply_migration` ile uygula**; `select column_name, is_generated from information_schema.columns where table_name='check_inler' and column_name in ('fotograf','fotograflar')` ile dogrula; `select count(*) from check_inler where fotograf is distinct from fotograflar[1]` = 0.
- [ ] **Step 3: Canli betigi guncelle ve kos**: `araclar/check-in-fotograf-degistir-canli-test.py` -> yeni RPC ile [yol1] -> [yol1,yol2] (kaldirilan bos) -> [yol2] (kaldirilan [yol1]) -> [] (kaldirilan [yol2]); 6 yol -> 'En fazla 5'; sarmalayici `check_in_fotografini_guncelle(id, yol)` eskiyi dondurur; baskasinin yolu/check-in'i/anon reddi. Beklenen: hepsi OK.
- [ ] **Step 4: `npm run test:sema`** yesil (check_in_yap null korumasi ve bulunurluk iddialari yeni imzayla calisir).
- [ ] **Step 5: Commit** `feat(db): check_inler.fotograflar (5'e kadar), fotograf generated, coklu fotograf RPC'leri`.
...
- [ ] Test: 3 fotografli ogede `akis-fotografi-0..2` var, rozet "1/3"; tek fotografta rozet yok; `onFotografAc(1)` ikinci kareye dokununca.
- [ ] Uygula; kart icindeki `duzenleAlani` JSX, taslak state'leri, picker fonksiyonlari, ArkadasSecici/IfadeSecici/SecimPenceresi kullanimlari ve stilleri sil; `menuVar = oge.benimMi && Boolean(onSilOnayi || onDuzenle)`.
- [ ] `npx jest __tests__/tasarim` yesil; commit `refactor(kart): fotograf seridi, yerinde duzenleme kalkti`.
...
- [ ] Test: galeri 2 secim -> 2 kare; kaldir; gonderince `checkInYap` 5. parametre `['uid/1.jpg','uid/2.jpg']`; yukleme hatasi uyari + fotografsiz devam (mevcut kural).
- [ ] Commit `feat(check-in): formda coklu fotograf`.
...
- [ ] `npx tsc --noEmit` uygulama kodunda 0 hata; `npx jest --runInBand` yesil.
- [ ] Commit `feat(ekranlar): duzenleme sayfasi + coklu fotograf galerileri`.

### Task 7: Dogrulama, belgeler, yayin

- [ ] `npm run test:gorunurluk` (senaryo 61/61b fotograf yollari - `fotograf` generated oldugu icin select ayni calismali; kirilirsa senaryo `fotograflar` okur).
- [ ] `python araclar/check-in-tekrar-canli-test.py`, `check-in-ifade-canli-test.py` (p_fotograf: None ile uyumlu).
- [ ] Ekran goruntusu: `node araclar/spa-sunucu.mjs dist` + `araclar/ekran-goruntusu.mjs` ile duzenleme sayfasi -> `tasarim/checkin-duzenle.png`.
- [ ] `docs/kvkk-uyum-listesi.md` ("5'e kadar fotograf"), `CLAUDE.md` bolumu, `slooin-projesi.md` kaldigi yer.
- [ ] `eas update --branch production --environment production --non-interactive`, `npm run yayinla`; commit + push.
