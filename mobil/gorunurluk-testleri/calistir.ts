...
async function main() {
  ...
  await senaryo('4 - gorunurluk = kimse olan ani gizlenir', async () => {
    ...
    // 'kimse' degerini yazan RPC (ani_gorunurlugunu_ayarla) 2026-09-20'de
    // KALDIRILDI (kullanicinin karari: tek gizlilik anahtari "Profilim
    // gizli"). Deger artik yalnizca gizli check-in'den donusumle olusur;
    // burada RLS kuralini olcmek icin servis roluyle yaziliyor.
    const yonetici = yoneticiIstemcisi()
    if (!yonetici) throw new Error('SUPABASE_SERVICE_ROLE_KEY yok; senaryo 4 servis rolu istiyor')
    const { error } = await yonetici
      .from('check_inler')
      .update({ gorunurluk: 'kimse' })
      .eq('id', aCheckIn1Id)
    if (error) throw new Error(`gorunurluk=kimse yazilamadi: ${error.message}`)
  ...
  await senaryo("31 - ani_gorunurlugunu_ayarla artik YOK", async () => {
    // 2026-09-20: "Anilarim kimlere gorunsun" ayari kaldirildi -
    // ekrani 09-19 ayarlar yeniden yaziminda menuden dusmustu, RPC ise
    // duruyor ve bir kullanicinin 29 anisini geri alinamaz bicimde
    // 'kimse' yapmisti. Tek gizlilik anahtari "Profilim gizli". RPC'nin
    // sunucudan gercekten dusuruldugu burada kilitli (PGRST202), yoksa
    // dogrudan cagriyla ayni tuzak yeniden kurulabilirdi.
    const { error } = await b.rpc('ani_gorunurlugunu_ayarla', { p_deger: 'herkese_acik' })
    esitMi(error?.code, 'PGRST202', 'ani_gorunurlugunu_ayarla sunucuda yok (PGRST202)')
  })
