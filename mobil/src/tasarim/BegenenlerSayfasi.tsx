...
import { begenenleriGetir, type Begenen } from '../../lib/etkilesim'
 ...
 * sayiya basilinca kimlerin begendigi gorunsun"). Yorum sayfasiyla ayni
 * kabuk: alttan gelen yarim sayfa, tutamac, baslik + x, liste. Satir:
...
export function BegenenlerSayfasi({
  ...
  const { t } = useDil()
  const [kisiler, setKisiler] = useState<Begenen[] | null>(null)
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onKapat}>
      <View style={stiller.kok}>
        <Pressable style={stiller.karartma} testID="begenenler-zemini" onPress={onKapat} />
        <View style={stiller.sayfa} testID="begenenler-sayfasi">
          ...
          {kisiler && kisiler.length === 0 && <Text style={stiller.durum}>{t('etkilesim.begenenYok')}</Text>}
