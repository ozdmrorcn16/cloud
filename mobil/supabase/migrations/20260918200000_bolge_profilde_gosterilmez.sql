-- OTURDUGU BOLGE PROFILDE HIC GOSTERILMEZ (2026-09-18 aksam).
-- Kullanicinin karari: "Oturdugun bolge kismini kaldir; bu secim sadece
-- hesap olusturma adiminda olacak, profilde gorunmeyecek ve profilde
-- gizleme gibi bir secenege de gerek kalmayacak; oturdugu bolgeyi
-- kullanicinin secmesi veri olarak saklanacak sadece."
--
-- Sabahki 20260918120000 migrasyonunun "bolge_gizli ile goster/gizle"
-- modeli boylece KALKIYOR. Sutunlar (yasadigi_ulke/il/ilce) ve
-- kayittaki secim DURUYOR; degisen yalnizca disariya akan yol:
--   * `baskasinin_profili` il/ilceyi artik HIC dondurmuyor (donus tipi
--     degistigi icin drop + create).
--   * `bolge_gizli` sutunu dusuyor - gosterim yolu yokken bir gizlilik
--     anahtari tutmak yaniltici olurdu.
--   * `verilerimi_disa_aktar` profil blokuna `yasadigi_ulke` eklendi:
--     KVKK m.11 - saklanan her alan kisinin kendi dosyasinda gorunmeli;
--     sabah eklenen ulke oraya girmemisti.
-- Kisi kendi satirini RLS ile okumaya devam ediyor; profil duzenle
-- ekrani artik bolgeyi okumuyor da yazmiyor.
...
alter table public.profiller drop column if exists bolge_gizli;

-- Disa aktarim: 20260913120000 dosyasindaki govdenin kopyasi, profil
-- blokuna yasadigi_ulke eklenmis hali. Sonraki degisiklik BU dosyadan
-- devam etmeli.
